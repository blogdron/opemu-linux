...
inline void _sstore_gpr32 (struct pt_regs * regs,ud_type_t n, uint32_t *where) {
...
inline void _sstore_gpr64 (struct pt_regs * regs,ud_type_t n, uint64_t *where) {
    /*TODO: check is correct*/
    switch (n) {
        case UD_R_RAX:
            *where = regs->ax;
...
int ssse3_grab_operands(ssse3_t *ssse3_obj)
{
    if (ssse3_obj->udo_dst->type == UD_OP_REG) {
        if (ssse3_obj->ismmx) {
            ...
            _sstore_gpr32 (ssse3_obj->op_obj->state,ssse3_obj->udo_dst->base, &ssse3_obj->dst.uint32[0]);
        }
    ...
    if (ssse3_obj->udo_src->type == UD_OP_REG) {
        ...
        printk("eee %s", ud_insn_asm(ssse3_obj->op_obj->ud_obj));
            _sstore_gpr32 (ssse3_obj->op_obj->state,ssse3_obj->udo_src->base, &ssse3_obj->src.uint32[0]);
        }
